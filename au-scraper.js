...
const BASE_SEARCH_URL =
    "https://www.seek.com.au/jobs?classification=1203%2C6281&daterange=2&salaryrange=90000-&salarytype=annual&subclassification=6178%2C6185%2C6287%2C6290%2C6294%2C6302&worktype=242%2C244";

// Replace with your n8n Production Webhook URL
const N8N_WEBHOOK_URL = "http://localhost:5678/webhook/seek-ingest";
...
async function runScraper() {
    console.log("🚀 Starting Deep Seek Australia Jobs Scraper...");
    ...
    // caching file
    const CACHE_FILE = "au_scraped_jobs_cache.json";
    ...
    if (fs.existsSync(CACHE_FILE)) {
        try {
            ...
            console.warn(
                "⚠️ Cache file was corrupted or empty. Starting a fresh cache.",
            );
        }
    }
    ...
    try {
        while (keepScraping) {
            ...
            try {
                ...
                console.log(
                    `No jobs found on page ${currentPage}. Reached the end of the search results.`,
                );
                keepScraping = false;
            ...
            console.log(
                `Found ${jobUrls.length} jobs on Page ${currentPage}. Beginning extraction...`,
            );
            const jobsData = [];
            ...
            for (let i = 0; i < jobUrls.length; i++) {
                ...
                console.log(
                    `[Page ${currentPage} - ${i + 1}/${jobUrls.length}] Scraping: ${canonicalUrl.split("?")[0]}`,
                ); // Clean URL logging
                ...
                try {
                    await detailPage.goto(canonicalUrl, {
                    ...
                    const title =
                        $('h1[data-automation="job-detail-title"]').text().trim() || null;
                    const company =
                        $('span[data-automation="advertiser-name"]').text().trim() || null;
                    const location =
                        $('span[data-automation="job-detail-location"]').text().trim() ||
                        null;
                    const employmentType =
                        $('span[data-automation="job-detail-work-type"]').text().trim() ||
                        null;
                    const description =
                        $('div[data-automation="jobAdDetails"]').text().trim() || null;
            ...
            if (jobsData.length > 0) {
                console.log(
                    `📦 Sending Batch of ${jobsData.length} jobs from Page ${currentPage} to n8n...`,
                );
                try {
